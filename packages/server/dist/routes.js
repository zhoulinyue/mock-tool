...
router.post('/api/projects/:id/rules', (req, res) => {
    const project = projectDB.getAll().find((p) => p.id === req.params.id);
    if (!project)
        return res.status(404).json({ error: 'project not found' });
...
router.put('/api/projects/:id/rules/:ruleId', (req, res) => {
    ...
    if (!project)
        return res.status(404).json({ error: 'project not found' });
    const rule = project.rules.find((r) => r.id === req.params.ruleId);
    if (!rule)
...
router.delete('/api/projects/:id/rules/:ruleId', (req, res) => {
    ...
    if (!project)
        return res.status(404).json({ error: 'project not found' });
    project.rules = project.rules.filter((r) => r.id !== req.params.ruleId);
    projectDB.update(req.params.id, { rules: project.rules });
    res.json({ success: true });
});
// =========== 获取某个项目下所有规则 ===========
